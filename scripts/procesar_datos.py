...
def main():
    ...
    if df2 is not None:
        for _, r in df2.iterrows():
            ...
            row = {
                ...
                "t_entrega": round(float(r.get("Tiempo de entrega",0) or 0), 2),
                "obj_meses": round(float(r.get("Objetivo_Meses de inventario",0) or 0), 2),
                "meses_inv": round(float(r.get("Meses de inventario",0) or 0), 2),
                ...
                "bo": round(float(r.get("Back Order",0) or 0), 2),
            }
    ...
    snap = {
        ...
        "detalleRows": [r for r in detalle_rows if r.get("fecha") == (uniq("fecha")[-1] if detalle_rows else None)],
    ...
    print(f"   📦 data.json: {size_kb:,.0f} KB ({size_mb:.1f} MB)")
    if size_mb > 90:
        print(f"   ⚠️  Archivo muy grande. Considera reducir el historial del Excel.")
